    ...
    for cur_state_ind = 1:machineStates.getLength,
        ...
        if stateExecuteFunctions.getLength > 0,
            for cur_exec_ind = 1:stateExecuteFunctions.getLength,
                ...
                cur_func.ParserName = '';
                cur_func.ParserCall = '';
                if ~isempty(stateExecuteFunctions.item(cur_exec_ind-1).getAttribute('Function')),
                    cur_func.Function = char(stateExecuteFunctions.item(cur_exec_ind-1).getAttribute('Function'));
                end
                if ~isempty(stateExecuteFunctions.item(cur_exec_ind-1).getAttribute('ParserName')),
                    cur_func.ParserName = char(stateExecuteFunctions.item(cur_exec_ind-1).getAttribute('ParserName'));
                end
                if ~isempty(stateExecuteFunctions.item(cur_exec_ind-1).getAttribute('ParserCall')),
                    cur_func.ParserCall = char(stateExecuteFunctions.item(cur_exec_ind-1).getAttribute('ParserCall'));
                end
